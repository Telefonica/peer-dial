...
let convertAdditionalData = (data) => {
  let values = decodeURIComponent(data).split('&'),
    ret = {};
...
var setupServer = function(){
  ...
  function handleRequest(req, res, next) {
	...
	app.use(pref, handleRequest);
  app.use('/apps/:appName/dial_data', handleRequest);
  ...
  function handleDialData(req, rsp) {
		var baseURL = req.protocol + "://" + (req.hostname || req.ip || self.host)+":"+self.port+pref;
		var appName = req.params["appName"];
		var app = self.delegate.getApp.call(req,appName);
		if (!app) {
      ...
      app.additionalData = convertAdditionalData(req.text) || {};
      //app.additionalData = JSON.parse(req.body);
      //self.delegate.dispatch.call(req,appName,req.body);
			rsp.sendStatus(501);
		}
  }
	app.post(pref+"/apps/:appName/dial_data",function(req,rsp){
    handleDialData(req, rsp)
  });
	app.post("/apps/:appName/dial_data",function(req,rsp){
    handleDialData(req, rsp)
	});  
